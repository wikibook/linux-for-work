...
## 서비스를 처리할 유니콘 워커의 개수를 지정한다. 기본 워커는 4지만 사용자가 많지 않다면 2 정도로 
## 설정해도 된다. 많은 부하가 걸리는 서버라면 3 또는 4로 지정한다. 
### worker를 3으로 지정할 경우 최소 2GB 이상의 램이 있어야 하며 4GB 이상일 경우 worker를 4로 지정한다.
worker_processes 4
...
## 유니콘 pid가 저장되는 디렉터리 경로를 설정한다. 해당 디렉터리가 존재해야 에러가 발생하지 않는다.
